package com.github.pleier.controller.system.head;
...
import com.github.pleier.util.mail.SimpleMailSender;
...
public class HeadController extends BaseController{
    ...
    public Object getList() {
        ...
        try {
            ...
            pdList.add(pds);
            map.put("list", pdList);
            map.put("plsmsCount", plsmsService.findPlsmsCount(Jurisdiction.getUsername()).get("plsmsCount").toString());//站内信未读总数
            String strWEBSOCKET = Tools.readTxtFile(Const.WEBSOCKET);//读取WEBSOCKET配置
            if(null != strWEBSOCKET && !"".equals(strWEBSOCKET)){
                String strIW[] = strWEBSOCKET.split(",pl,");
                if(strIW.length == 5){
                    map.put("wimadress", strIW[0]+":"+strIW[1]);	//即时聊天服务器IP和端口
                    map.put("oladress", strIW[2]+":"+strIW[3]);		//在线管理和站内信服务器IP和端口
                    map.put("PLsmsSound", strIW[4]);				//站内信提示音效配置
                }
            }
        }catch (Exception e){
     ...
     * @return
     */
    @RequestMapping(value="/getPlsmsCount")
    @ResponseBody
    public Object getPlsmsCount() {
        PageData pd = new PageData();
        Map<String,Object> map = new HashMap<String,Object>();
        try {
            map.put("plsmsCount", plsmsService.findPlsmsCount(Jurisdiction.getUsername()).get("plsmsCount").toString());//站内信未读总数
    ...
    public Object sendEmail(){
        ...
        if(null != strEMAIL && !"".equals(strEMAIL)){
            String strEM[] = strEMAIL.split(",pl,");
            if(strEM.length == 4){
                if("yes".endsWith(isAll)){
                    try {
                        List<PageData> userList = new ArrayList<PageData>();
    ...
    public ModelAndView goEditEmail() throws Exception{
        ...
        if(null != strFWATERM && !"".equals(strFWATERM)){
            String strFW[] = strFWATERM.split(",pl,");
            if(strFW.length == 5){
    ...
    public ModelAndView saveSys() throws Exception{
        ...
        Tools.writeFile(Const.EMAIL,pd.getString("SMTP")+",pl,"+pd.getString("PORT")+",pl,"+pd.getString("EMAIL")+",pl,"+pd.getString("PAW"));	//写入邮件服务器配置
        Tools.writeFile(Const.SMS1,pd.getString("SMSU1")+",pl,"+pd.getString("SMSPAW1"));	//写入短信1配置
        Tools.writeFile(Const.SMS2,pd.getString("SMSU2")+",pl,"+pd.getString("SMSPAW2"));	//写入短信2配置
        mv.addObject("msg","OK");
    ...
    public ModelAndView saveSys2() throws Exception{
        ...
        ModelAndView mv = this.getModelAndView();
        PageData pd = this.getPageData();
        Tools.writeFile(Const.FWATERM,pd.getString("isCheck1")+",pl,"+pd.getString("fcontent")+",pl,"+pd.getString("fontSize")+",pl,"+pd.getString("fontX")+",pl,"+pd.getString("fontY"));	//文字水印配置
        Tools.writeFile(Const.IWATERM,pd.getString("isCheck2")+",pl,"+pd.getString("imgUrl")+",pl,"+pd.getString("imgX")+",pl,"+pd.getString("imgY"));	//图片水印配置
        Watermark.fushValue();
        mv.addObject("msg","OK");
        mv.setViewName("save_result");
        return mv;
    ...
    public ModelAndView saveSys3() throws Exception{
        ...
        Tools.writeFile(Const.WEBSOCKET,pd.getString("WIMIP")+",pl,"+pd.getString("WIMPORT")+",pl,"+pd.getString("OLIP")+",pl,"+pd.getString("OLPORT")+",pl,"+pd.getString("PLsmsSound"));	//websocket配置
        mv.addObject("msg","OK");
        mv.setViewName("save_result");
        return mv;
    }
